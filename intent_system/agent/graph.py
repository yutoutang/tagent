...
from intent_system.agent.nodes import (
    intent_parse_node,
...
def route_after_parse(state: UnifiedAgentState) -> Literal["orchestrate", "synthesize"]:
    """
    解析后路由决策

    如果检测到意图，进入编排；否则直接综合
    """
    if state.detected_intents:
        return "orchestrate"
    return "synthesize"


def route_after_orchestrate(state: UnifiedAgentState) -> Literal["execute", "synthesize"]:
    """
    编排后路由决策
...
def route_after_execute(state: UnifiedAgentState) -> Literal["reflect", "synthesize"]:
    """
    执行后路由决策
...
def route_after_reflect(state: UnifiedAgentState) -> Literal["execute", "synthesize"]:
    """
    反思后路由决策

    根据反思结果决定是否继续执行还是综合
    """
    if state.reflection_result and state.reflection_result.should_continue:
        # 重置执行状态，重新执行
        return "execute"
    return "synthesize"


def create_unified_agent_graph(
    llm=None,
    ...
    # 添加节点
    graph.add_node("intent_parse", lambda state: intent_parse_node(state, config))
    graph.add_node("orchestrate", lambda state: intent_orchestrate_node(state, config))
    graph.add_node("execute", lambda state: intent_execute_node(state, config))
    graph.add_node("reflect", lambda state: reflect_node(state, config))
    graph.add_node("synthesize", lambda state: synthesize_node(state, config))
