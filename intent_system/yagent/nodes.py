...
from intent_system.yagent.state import (
    YAgentState,
    ReflectionResult,
...
def intent_parse_node(state: YAgentState, config: Dict[str, Any]) -> Dict[str, Any]:
    """
    意图解析节点
    ...
    if not llm:
        # 使用简单的关键词匹配作为降级方案
        all_intents = registry.list_all()
        if all_intents:
            # 使用第一个意图作为默认
            default_intent = all_intents[0].metadata.id
            return {
                "detected_intents": [default_intent],
                ...
                "intermediate_steps": [{
                    ...
                    "confidence": 0.3,
                    "reasoning": "LLM未配置，使用默认意图"
                }],
                "errors": ["LLM未配置，使用降级模式"]
            }
        return {
            "detected_intents": [],
            "intent_confidence": 0.0,
            "errors": ["LLM未配置，且无可用意图"]
        }
...
def intent_orchestrate_node(state: YAgentState, config: Dict[str, Any]) -> Dict[str, Any]:
    """
    意图编排节点
...
async def intent_execute_node(state: YAgentState, config: Dict[str, Any]) -> Dict[str, Any]:
    """
    意图执行节点
...
def reflect_node(state: YAgentState, config: Dict[str, Any]) -> Dict[str, Any]:
    """
    反思节点
    ...
    if not llm:
        ...
        return {
            "reflection_result": ReflectionResult(
                should_continue=False,  # 无LLM时不继续迭代
                confidence=0.5,
                reasoning="LLM未配置，跳过反思评估"
            ),
            "is_complete": True,
            "iteration": iteration + 1,
            "errors": state.errors + ["LLM未配置，使用简单逻辑"]
        }
    ...
    try:
        # 使用流式调用
        print(f"\n[LLM 调用] 反思评估 (迭代 {iteration + 1}/{max_iterations})...")
        response_content = ""
...
def _build_reflection_prompt(state: YAgentState) -> str:
    """构建反思提示"""
    prompt = f"""请评估以下执行结果：
...
def synthesize_node(state: YAgentState, config: Dict[str, Any]) -> Dict[str, Any]:
    """
    综合节点
    ...
    if not llm:
        ...
        return {
            ...
            "is_complete": True,
            "errors": state.errors + ["LLM未配置，使用简单格式"]
        }
...
def _build_synthesis_prompt(state: YAgentState) -> str:
    """构建综合提示"""
...
def _simple_synthesis(state: YAgentState) -> str:
    """简单综合（备用方案）"""
    lines = ["# 执行结果\n"]
