...
# todo 这个方法抽取出来，有些太冗余了
def default_convert_to_llm(messages: list[AgentMessage]) -> list[Message]:
    """
    Default convertToLlm: Keep only LLM-compatible messages, convert attachments.
    Handles conversion from dict to dataclass for nested content fields.
    """
    transfer_messages = []
    for m in messages:
        role = m.get("role")
        if role == "user":
            # Handle content conversion for UserMessage
            content = m.get("content", "")
            timestamp = m.get("timestamp")

            # Convert list of dict content to proper types if needed
            ...
            transfer_messages.append(UserMessage(
                role="user",
                content=content,
            ...
            # Handle content conversion for AssistantMessage
            content = m.get("content", [])
            timestamp = m.get("timestamp")

            # Convert list of dict content to proper types
            if isinstance(content, list):
                converted_content = []
                for item in content:
                    if isinstance(item, dict):
                        ...
                        if item_type == "text":
                            ...
                            converted_content.append(ThinkingContent(
                                type="thinking",
                            ...
                            converted_content.append(ToolCall(
                                type="toolCall",
                                id=item.get("id", ""),
                                name=item.get("name", ""),
            ...
            usage_dict = m.get("usage", {})
            if isinstance(usage_dict, dict):
                from ..ai.types import Usage, UsageCost
                cost_dict = usage_dict.get("cost", {})
                usage = Usage(
                    input=usage_dict.get("input", 0),
                    output=usage_dict.get("output", 0),
                    cacheRead=usage_dict.get("cacheRead", 0),
                    cacheWrite=usage_dict.get("cacheWrite", 0),
                    totalTokens=usage_dict.get("totalTokens", 0),
                    cost=UsageCost(
            ...
            transfer_messages.append(AssistantMessage(
                role="assistant",
                content=content,
                api=m.get("api", ""),
                ...
                model=m.get("model", ""),
                usage=usage,
                stopReason=m.get("stopReason", "stop"),
                errorMessage=m.get("errorMessage"),
                timestamp=timestamp if timestamp else int(time.time() * 1000),
            ))

        elif role == "toolResult":
            # Handle content conversion for ToolResultMessage
            content = m.get("content", [])
            timestamp = m.get("timestamp")
            ...
            transfer_messages.append(ToolResultMessage(
                role="toolResult",
